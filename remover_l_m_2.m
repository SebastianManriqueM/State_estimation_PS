...
for i = 1 : size(no_lobs_i,1)                               %Remover da Rede linhas com fluxo != 0
    ...
    if ( abs( S_ij_obs(no_lobs_i(a), no_lobs_j(a)) ) > 1e-10 )
        ...
        for j = 1 : size(no_mobs_i,1)                       %Remover medidas de Pinj adjacentes
            b = j - contadorb;
            if tipo_mobs(b) == 2
                if(no_mobs_i(b) == no_lobs_i(a)) || (no_mobs_i(b) == no_lobs_j(a))
                    no_mobs_i = [no_mobs_i(1:b-1, :) ;no_mobs_i(b+1:size(no_mobs_i,1), :) ];
                    no_mobs_j = [no_mobs_j(1:b-1, :) ;no_mobs_j(b+1:size(no_mobs_j,1), :) ];
                    num_tipo_mobs(2) = num_tipo_mobs(2) - 1;
                    tipo_mobs = [tipo_mobs(1:b-1, :) ;tipo_mobs(b+1:size(tipo_mobs,1), :) ];
